    ...
    public static class CacheAdvisor {
        ...
        public static Object cache(
            ...
            if (resultInCache != null) {
                // ne c'è cache
                if (isCacheExpired(resultInCache, method)) {
                    // cache expired
                    return invokeActualMethodAndPutIntoCache(superCall, cacheKey);
                } else { // not expired
        ...
        private static Object invokeActualMethodAndPutIntoCache(@SuperCall Callable<Object> superCall, CacheKey cacheKey) throws Exception {
            Object actualMethodInvocationResult = superCall.call();
            cache.put(cacheKey, new CacheValue(actualMethodInvocationResult, System.currentTimeMillis()));
            return actualMethodInvocationResult;
        }

        private static boolean isCacheExpired(CacheValue value, Method method) {
            long time = value.time;
            int cacheAnnotationValueInSeconds = method.getAnnotation(Cache.class).cacheSeconds();

            return System.currentTimeMillis() - time > cacheAnnotationValueInSeconds * 1000;
        }
    }
